...
class MoveStraight(Node):
    def __init__(self):
        ...
        )  # m/s (positive = forward)
        self.declare_parameter(
            "timeout", robot_motion_timeout_duration
        )  # seconds
    ...
    def run_straight(self):
        ...
        while not self.odom_received and (time.time() - start) < odom_timeout:
            ...
            )  # timeout sec is the time ros2 takes to wait for a command
        ...
        self.get_logger().info(
            f"Starting to move {direction} at {abs(linear_speed):.2f} m/s"
        )
        ...
        self.get_logger().info(
            f"Total distance traveled: {total_distance:.3f} m"
        )
